...
def generate_launch_description():
    ...
    # NOTE: champ_base's own leg-kinematics odometry (robot_localization ekf_node
    # chain: base_to_footprint_ekf + footprint_to_odom_ekf, fed by /odom/raw)
    # diverges to huge garbage values for this go2 port and corrupts SLAM/Nav2's
    # TF tree. In simulation we bypass it entirely and publish odom->base_link
    # straight from Gazebo's ground-truth odometry instead.
    ground_truth_odom_bridge = Node(
        package="champ_base",
        executable="ground_truth_odom_bridge.py",
        output="screen",
        parameters=[
            {"use_sim_time": LaunchConfiguration("use_sim_time")},
        ],
